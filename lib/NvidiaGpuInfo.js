...
class NvidiaGpuInfo {
     ...
     */
    getCoreNumbers() {
        return this._coreNumbers.values();
    }
     ...
     * @returns {boolean}
     */
    hasCoreNumber(coreNumber) {
        return this._coreNumbers.has(coreNumber);
    }
    ...
    async parseGpuMetaData() {
        ...
        try {
            ...
            while ((matchResult = DATA_GRAB_PATTERN.exec(coresMetaData)) !== null) {
                const coreNumber = Number.parseInt(matchResult[2]);
                this._coreNumbers.add(coreNumber);
                this._productNames[coreNumber] = matchResult[1];
                this._totalMemory[coreNumber] = Number.parseInt(matchResult[3]);
            }
